...
class _EditPeriodState extends State<EditPeriod> {
  ...
  Future<void> _showDateOrderError(String message) async {
    await showDialog<void>(
      context: context,
      barrierDismissible: false,
      builder: (dialogContext) => AlertDialog(
        title: const Text('Error en las fechas del período'),
        content: Row(
          ...
          children: [
            ...
            Expanded(
              child: Text('$message Debes corregirla para continuar.'),
            ),
          ],
        ),
        actions: [
  ...
  Future<void> _selectStartDate() async {
    ...
    while (true) {
      ...
      final warning = validatePeriodStartDate(
        startDate: picked,
        endDate: _endDate,
      );
      if (warning == null) {
      ...
      await _showDateOrderError(warning);
      initialDate = picked;
    }
  }

  Future<void> _selectEndDate() async {
    DateTime initialDate = _endDate ?? (_startDate ?? DateTime.now());
    ...
    while (true) {
      ...
      final warning = validatePeriodEndDate(
        endDate: picked,
  ...
  void _onSave() {
    ...
    final dateOrderError = _startDate != null && _endDate != null
          ...
          title: Row(
            children: [
              Icon(Icons.warning_amber_rounded,
                  color: colors.iconDeleteColor, size: 28),
              const SizedBox(width: 8),
              Expanded(
                child: Text(
                  matterCount > 0
                      ? '¿Eliminar período en uso?'
                      : '¿Eliminar período?',
                  style: TextStyle(
              ...
              children: [
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    hintStyle: TextStyle(
                        color: colors.editDimensionText.withValues(alpha: 0.6)),
                    prefixIcon:
                        Icon(Icons.school_outlined, color: colors.drawerButton),
